...
import static com.dev.damagehandler.buff.Buff.mapBuffData;
...
public class BuffData {
    ...
    public void addBuff(BuffStatus buff) {
        totalBuff.add(buff);
        if (!mapBuffData.containsKey(this.uuid)) mapBuffData.put(this.uuid, this);
    }
    ...
    public void removeBuff(UUID uuid) {
        for (BuffStatus buff : totalBuff) {
            if (buff.getUniqueId() == uuid) {
                ...
                if (totalBuff.isEmpty()) mapBuffData.remove(this.uuid);
                break;
            }
        }
    }
